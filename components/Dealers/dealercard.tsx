...
const DealerCard = React.memo(({ dealers }: any) => {
  // Memoize image source URL
  const imageUrl = useMemo(() => dealers.image, [dealers.image]);

  // Memoize dealer detail URL
  const dealerDetailUrl = useMemo(() => dealers.detailUrl, [dealers.detailUrl]);

  return (
    <Card
      className='md:h-66 mx-auto mt-6 h-52 sm:h-72 md:mt-4 lg:mb-4 lg:mt-2 lg:h-64'
    >
      <CardHeader
        floated={true}
        className='h-26 mx-2 -mt-2 shadow-none sm:-mt-4 sm:h-36 md:-mt-3 md:h-40 lg:-mt-7 lg:h-40'
      >
        <Link target='_blank' href={dealerDetailUrl} passHref prefetch={false}>
          <Image
            src={imageUrl}
            alt=''
        ...
        <Typography variant='h6' color='blue-gray'>
          ...
          <div className='mt-1 flex items-center justify-center md:mt-0 lg:mt-0'>
            {[...Array(5)].map((_, index) => {
              ...
              return (
                <span key={index}>
                  {dealers.rating >= index + 1 || dealers.rating >= numbers ? (
                    <TiStarFullOutline
                      fill='#E49E2F'
